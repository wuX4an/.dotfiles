...
from .modules.widgets.battery import percent, battery
...
def open_rmpc():
    qtile.cmd_spawn(
        f"{open_in_term} rmpc -c {settings['dirs']['CONFIG']}/rmpc/config.ron"
    )


def open_nmpy():
    qtile.cmd_spawn(
        f"{open_in_term} python3 {settings['dirs']['SETTINGS']}/modules/network/fzf.py"
    )
